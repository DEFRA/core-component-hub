...
> [!WARNING]
> The `core-component-hub` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `core-component-hub`. To turn it
...
```bash
docker build --target development --no-cache --tag core-component-hub:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 core-component-hub:development
```
...
```bash
docker build --no-cache --tag core-component-hub .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 core-component-hub
```
...
```bash
docker run -it --entrypoint /bin/ash core-component-hub
```
